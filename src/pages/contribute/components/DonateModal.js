    ...
    switch (type) {
      ...
      case ACTION_UPDATE_ALLOWANCE: {
        ...
        return {
          ...state,
          allowance,
          viewState: amountBN.gt(toBN(allowance))
            ? VIEW_READY_TO_APPROVE
            : VIEW_ENOUGH_ALLOWANCE
        }
      }
...
const DonateModal = props => {
  ...
  const updateAllowance = async () => {
    return daiTokenContract
      ...
      .call()
      .then(value =>
        dispatch({ type: ACTION_UPDATE_ALLOWANCE, allowance: value })
      )
  ...
  contents[VIEW_READY_TO_APPROVE] = (
    <>
      <h2 className='mb-2'>
        You first need to approve access to your DAI balance before you can
        donate
      </h2>
  ...
  const showApproveButton = [VIEW_APPROVING, VIEW_READY_TO_APPROVE].includes(
    viewState
  )
          ...
          {showApproveButton && (
            <button
              className={`button is-primary ${
                viewState === VIEW_APPROVING ? 'is-loading' : ''
              }`}
          ...
          {showDonateButton && (
            <button
              className={`button is-success ${
                viewState === VIEW_DONATING ? 'is-loading' : ''
              }`}
